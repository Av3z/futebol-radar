        ...
        builder: (context, state) {
          if (state is NewsSearchInitial || state is SourcesLoaded) {
            return const EmptyState(
                icon: Icons.search,
                message: 'Configure os filtros para começar uma pesquisa.');
          }
          ...
          if (state is NewsSearchEmpty) {
            return const EmptyState(
                ...
                message: 'Nenhuma notícia encontrada para estes filtros.');
          }
          if (state is NewsSearchFailure) {
            return EmptyState(
              icon: Icons.cloud_off,
              message: state.message,
              action: state.filters is SearchFilters
                  ? () => context
                      .read<NewsSearchBloc>()
                      .add(NewsSearchRetried(state.filters!))
                  : null,
            );
          }
